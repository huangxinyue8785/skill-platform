...
const cityToProvince = {
    ...
    '咸宁': '湖北', '随州': '湖北', '恩施土家族苗族自治州': '湖北', '仙桃': '湖北',
    '潜江': '湖北', '天门': '湖北',

    // 湖南省
    '长沙': '湖南', '株洲': '湖南', '湘潭': '湖南', '衡阳': '湖南', '邵阳': '湖南',
    '岳阳': '湖南', '常德': '湖南', '张家界': '湖南', '益阳': '湖南', '郴州': '湖南',
    '永州': '湖南', '怀化': '湖南', '娄底': '湖南', '湘西土家族苗族自治州': '湖南',
    ...
    // 海南省
    '海口': '海南', '三亚': '海南', '三沙': '海南', '儋州': '海南', '文昌': '海南',
    '琼海': '海南', '万宁': '海南', '东方': '海南', '五指山': '海南', '澄迈县': '海南',
    '陵水黎族自治县': '海南',
    ...
    '雅安': '四川', '巴中': '四川', '资阳': '四川', '阿坝藏族羌族自治州': '四川',
    '甘孜藏族自治州': '四川', '凉山彝族自治州': '四川', '西昌': '四川',

    // 贵州省
    '贵阳': '贵州', '六盘水': '贵州', '遵义': '贵州', '安顺': '贵州', '毕节': '贵州',
    '铜仁': '贵州', '黔西南布依族苗族自治州': '贵州', '黔东南苗族侗族自治州': '贵州',
    '黔南布依族苗族自治州': '贵州',

    // 云南省
    '昆明': '云南', '曲靖': '云南', '玉溪': '云南', '保山': '云南', '昭通': '云南',
    '丽江': '云南', '普洱': '云南', '临沧': '云南', '楚雄彝族自治州': '云南',
    '红河哈尼族彝族自治州': '云南', '文山壮族苗族自治州': '云南',
    '西双版纳傣族自治州': '云南', '大理白族自治州': '云南',
    '德宏傣族景颇族自治州': '云南', '怒江傈僳族自治州': '云南',
    '迪庆藏族自治州': '云南',
    ...
    '定西': '甘肃', '陇南': '甘肃', '临夏州': '甘肃', '甘南藏族自治州': '甘肃',

    // 青海省
    '西宁': '青海', '海东': '青海', '海北藏族自治州': '青海', '黄南藏族自治州': '青海',
    '海南藏族自治州': '青海', '果洛藏族自治州': '青海', '玉树藏族自治州': '青海',
    '海西蒙古族藏族自治州': '青海',
    ...
    '阿克苏地区': '新疆', '克孜勒苏柯尔克孜自治州': '新疆', '喀什地区': '新疆',
    '和田地区': '新疆', '伊犁哈萨克自治州': '新疆', '塔城地区': '新疆',
    '阿勒泰地区': '新疆', '石河子': '新疆', '阿拉尔': '新疆', '图木舒克': '新疆',
    '五家渠': '新疆', '铁门关': '新疆', '双河': '新疆', '可克达拉': '新疆',
    '昆玉': '新疆', '新星': '新疆'
};

console.log('📚 开始处理普通高校数据...');

// 读取 Excel 文件
const workbook = XLSX.readFile('W020250729615142156867.xls');
const sheet = workbook.Sheets['全国普通高等学校名单'];
...
for (let i = 3; i < data.length; i++) {
    ...
    const name = row[1]; // 学校名称
    const location = row[4]; // 所在地

    if (!location) continue;

    // 处理城市名（去掉"市"字）
    let city = location.replace('市', '');

    // 从映射字典获取省份
    let province = cityToProvince[city];
    ...
    values.push(`('${name.replace(/'/g, "''")}', '${province}', '${city}')`);
}

// 写入 SQL 文件
sql += values.join(',\n') + ';';

fs.writeFileSync('schools_2025.sql', sql);

console.log(`✅ 生成完成！共 ${values.length} 条数据`);
console.log(`📁 文件位置: schools_2025.sql`);

if (missedCities.size > 0) {
    console.log('\n⚠️ 以下城市未找到对应省份，已用城市名代替：');
    console.log(Array.from(missedCities).sort().join(', '));
}